 ...
 */
export type TargetRef<T extends EventTarget = EventTarget> =
  | T
  | null
 ...
 */
export type InferTargetRef<T> = T extends string
  ? Element
  : T extends object & { current: infer R | null }
  ? R
  : NonNullable<T>
 ...
 */
export function getElement<K extends keyof SVGElementTagNameMap>(
  selectors: K | null
 ...
 */
export function getElement<T extends TargetRef | string>(
  refOrElement: T
): InferTargetRef<T> | null
export function getElement<T extends EventTarget>(
  selectorsOrElementOrRef: TargetRef<T> | string
) {
...
export function resolveState<T, E extends T>(
  state: StateSetter<T>,
  current: E
): T
export function resolveState<T, E extends T>(
