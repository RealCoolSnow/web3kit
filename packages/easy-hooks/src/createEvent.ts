...
export type SupportEventTarget =
  | Window
  | Element
  | Document
  | EventSource
...
export type EventMap<T extends SupportEventTarget> = T extends Window
  ? WindowEventMap
  : T extends Document
  ? DocumentEventMap
...
export type EventNameMap<T extends SupportEventTarget> = keyof EventMap<T>

export type EventListenerMap<
  T extends SupportEventTarget,
  K extends EventNameMap<T>
...
export const createEvent: CreateEvent = function createEvent(
  ...
  if (element) {
    eventNames.forEach((name) =>
      element.addEventListener(name, eventListener as any, options)
    )
    return () =>
      eventNames.forEach((name) =>
        element.removeEventListener(name, eventListener as any, options)
      )
  }
}
